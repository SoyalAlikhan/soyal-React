...
function App() {
  ...
  const handleSchedulerSubmit = (e) => {
    ...
    if (typeof window !== 'undefined' && window.apiService && window.apiService.createBatch) {
      window.apiService.createBatch({
        title: schedulerModal.courseTitle + " (" + schedulerModal.recurrence + ")",
        schedule_days: schedulerModal.recurrence || 'Mon, Wed, Fri',
        class_time: schedulerModal.startTime || '08:30 PM PKT',
        max_talaba: 30
      }).then(res => {
        if (res && res.success) console.log('[SQLite Backend] Batch schedule saved to DB:', res.data);
      });
    }
