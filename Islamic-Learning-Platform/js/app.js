  ...
  return (
    <div className={lang === 'ur' ? 'urdu-mode' : ''} dir={lang === 'ur' ? 'rtl' : 'ltr'}>
      ...
      <main className="main-content">
        ...
        {/* =========================================================================
            2. TEACHER STUDIO & MINI ACADEMY (ALL TEACHER FORMS)
           ========================================================================= */}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  <button className="btn btn-gold" onClick={() => setCourseStudio({ ...courseStudio, isOpen: true })}>
                    <i className="fas fa-plus-circle"></i> Create Course Studio (BRD Flow 3)
                  </button>
                  <button className="btn btn-primary" onClick={() => setSchedulerModal({ ...schedulerModal, isOpen: true })}>
                    <i className="fas fa-calendar-plus"></i> Schedule Recurring Class (Flow 5)
                  </button>
                  <button className="btn btn-outline" onClick={() => setHwCreateModal({ ...hwCreateModal, isOpen: true })}>
                    <i className="fas fa-tasks"></i> Create Homework (Section 10)
                  </button>
                  <button className="btn btn-outline" onClick={() => setHifzEntryModal({ ...hifzEntryModal, isOpen: true })}>
                    <i className="fas fa-book-reader"></i> Daily Hifz Entry (Section 11)
                  </button>
                </div>
              </div>
            </div>

            {/* LIVE AUTO ATTENDANCE SIMULATOR & OVERRIDE */}
            <div className="attendance-sim-box">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="btn btn-outline btn-sm" onClick={() => alert("Simulated net glitch: Ahmad Raza disconnected -> reconnected.")}>
                    Simulate Net Glitch
                  </button>
                  <button className="btn btn-primary btn-sm" onClick={() => alert("Class ended. Attendance percentage calculated automatically per BRD Flow 6!")}>
                    Finalize Attendance
                  </button>
                </div>
              </div>
                ...
                <tbody>
                  {attendanceEngine.sessions.map(sess => (
                    <tr key={sess.id}>
                      ...
                      <td>{sess.flag === 'network_unstable' ? <span className="badge badge-gold">Net Unstable</span> : <span>—</span>}</td>
                      <td>
                        <button 
            ...
            <div className="glass-card" style={{ padding: '24px', margin: '24px 0' }}>
              ...
              {audioSubmissions.map(sub => (
                <div key={sub.id} style={{ padding: '16px', background: 'rgba(0,0,0,0.25)', borderRadius: '12px', marginBottom: '14px', border: '1px solid var(--border-subtle)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--color-primary-light)' }}>{sub.studentName}</div>
                      <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{sub.courseTitle} • {sub.lessonTitle}</div>
                    </div>
                  ...
                  <button 
                    ...
                    onClick={() => {
                      const note = prompt("Enter teacher feedback:", "MashaAllah, good recitation. Keep proper Ghunnah duration.");
                      if (note) {
                  ...
                  >
                    Grade Recitation & Submit Voice/Text
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="glass-card" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.25)', padding: '14px', borderRadius: '10px' }}>
                  ...
                  <button className="btn btn-outline btn-sm" style={{ marginTop: '8px' }} onClick={() => alert("Payout request submitted to platform accounts!")}>
                    Request Payout
                  </button>
                </div>
              </div>
                  ...
                  {courseStudio.modules.map(mod => (
                    <div key={mod.id} className="module-builder-box">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ fontWeight: 700, color: 'var(--color-primary-light)' }}>{mod.title}</div>
                        <button type="button" className="btn btn-primary btn-sm" style={{ padding: '3px 8px', fontSize: '0.72rem' }} onClick={() => handleAddLesson(mod.id)}>
                          + Add Lesson
                        </button>
                      </div>
                      ...
                      {mod.lessons.map(les => (
                        <div key={les.id} className="lesson-item-row">
                          <i className={`fas ${les.type === 'Video' ? 'fa-video' : les.type === 'Audio' ? 'fa-headphones' : 'fa-file-alt'}`} style={{ color: 'var(--color-accent-gold)' }}></i>
                          <div style={{ flex: 1 }}>
        ...
        <div className="auth-overlay" onClick={() => setCertModal({ ...certModal, isOpen: false })}>
          <div className="auth-modal" style={{ maxWidth: '650px', border: '2px solid var(--border-gold)' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ textAlign: 'center', border: '2px dashed var(--border-gold)', padding: '24px', borderRadius: '14px' }}>
              <div style={{ fontSize: '1.8rem', color: 'var(--color-accent-gold)' }}><i className="fas fa-certificate"></i></div>
              <h3 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--color-accent-gold)', margin: '6px 0' }}>Official Certificate of Completion</h3>
              <div style={{ margin: '14px 0' }}>
                <div>This certifies that</div>
                <div style={{ fontSize: '1.6rem', fontWeight: 800, color: '#ffffff' }}>{certModal.studentName}</div>
                <div>has mastered the curriculum of Noorani Qaida & Tajweed</div>
              </div>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Certificate ID: CERT-2026-00892</div>
            </div>
          </div>
        </div>
      )}
